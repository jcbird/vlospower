...
sim = st.load_faceon(400, config.params)
#2D velocities
sim['vcxy']
sim['vrxy']
...
radprofileSim = pynbody.analysis.profile.Profile(subSim, min=0.00, max=20.0, nbins=60)
...
im = ax.imshow(deltaVmap.T, aspect='auto',  interpolation='none', extent=[extentx[0], extentx[1], extenty[0], extenty[1]], cmap='Spectral', origin='lower', vmax=40, vmin=-40)
...
im2 = ax2.imshow(deltaV2map.T, aspect='auto',  interpolation='none', extent=[extentx[0], extentx[1], extenty[0], extenty[1]], cmap='Spectral', origin='lower', vmax=40, vmin=-40)
